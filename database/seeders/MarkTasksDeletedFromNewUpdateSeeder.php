...
{
    /**
     * Mark tasks as deleted if their directory is not listed in seeders/data/finalmdplist.json.
     */
    public function run(): void
    {
        ...
        if (!is_array($rows)) {
            $this->command->error('finalmdplist.json not valid JSON array.');
            return;
        }
        ...
        if ($validIdCards->isEmpty()) {
            $this->command->warn('No valid Id # values found in finalmdplist.json; no tasks will be marked deleted.');
            return;
        }
        ...
        if ($directoriesToMark->isEmpty()) {
            $this->command->info('No directories found outside finalmdplist.json; no tasks changed.');
            return;
        }

        $this->command->info('Found ' . $directoriesToMark->count() . ' directories not in finalmdplist.json. Marking related tasks as deleted...');
            ...
            ->chunkById(500, function ($tasks) use (&$totalTasks, &$updatedTasks) {
                foreach ($tasks as $task) {
                    ...
                    EventLog::create([
                        ...
                        'description' => 'Task soft-deleted via MarkTasksDeletedFromNewUpdateSeeder because directory is not in finalmdplist.json 03/01/2026',
                        'event_data' => [
