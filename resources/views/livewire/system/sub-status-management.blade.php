    ...
    <div class="toolbar" id="kt_toolbar">
        <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
            <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                <h1 class="text-dark fw-bold my-1 fs-2">Sub Status Types</h1>
                <ul class="breadcrumb fw-semibold fs-base my-1">
                    <li class="breadcrumb-item text-muted"><a href="#" class="text-muted text-hover-primary">System</a></li>
                    <li class="breadcrumb-item text-dark">Sub Status Types</li>
                </ul>
            </div>
        </div>
    </div>
                ...
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" wire:model.live.debounce.500ms="search" class="form-control form-control-solid w-250px ps-13" placeholder="Search sub statuses">
                        </div>
                    </div>
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-ss-table-toolbar="base">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5">
                            ...
                            <tbody class="text-gray-600 fw-semibold">
                                @forelse($statuses as $s)
                                    <tr @class(['table-active'=>$editId===$s->id])>
                                        <td>
                                            @if($editId === $s->id)
                                                <input type="text" class="form-control form-control-sm" wire:model.defer="editName" />
                                            @else
                                                <span class="text-gray-800 text-hover-primary fw-bold">{{ $s->name }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($editId === $s->id)
                                        ...
                                        <td><span class="badge {{ $s->active ? 'badge-light-success' : 'badge-light-danger' }}">{{ $s->active ? 'Active' : 'Inactive' }}</span></td>
                                        <td class="text-end">
                                            @if($editId === $s->id)
                                            ...
                                            @else
                                                <button class="btn btn-sm btn-light-primary" wire:click="edit('{{ $s->id }}')">Edit</button>
                                                <button class="btn btn-sm {{ $s->active ? 'btn-danger' : 'btn-success' }}" wire:click="toggle('{{ $s->id }}')" wire:loading.attr="disabled">{{ $s->active ? 'Deactivate' : 'Activate' }}</button>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="4" class="text-center text-muted py-10">No sub statuses found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    <div class="d-flex justify-content-end mt-4">{{ $statuses->links('vendor.pagination.new') }}</div>
                </div>
            </div>
        </div>
    </div>
